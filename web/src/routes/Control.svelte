<script lang="ts">
  ...
  function sendConfig() { client?.sendConfig({ scale, bitrate_kbps: bitrate, fps, codec, privacy }); }

  // 鼠标/键盘/触屏事件（与 Connect 共享；这里简化以避免重复代码）
  ...
  function onWheel(e: WheelEvent) { e.preventDefault(); client?.sendCmd({ op: 'wheel', dx: e.deltaX, dy: e.deltaY }); }
  ...
  function onKeyDown(e: KeyboardEvent) { if (!client) return; e.preventDefault(); client.sendCmd({ op: 'key', code: e.code, key: e.key, down: true, mods: { ctrl: e.ctrlKey, alt: e.altKey, shift: e.shiftKey, meta: e.metaKey } }); }
  function onKeyUp(e: KeyboardEvent) { if (!client) return; e.preventDefault(); client.sendCmd({ op: 'key', code: e.code, key: e.key, down: false, mods: { ctrl: e.ctrlKey, alt: e.altKey, shift: e.shiftKey, meta: e.metaKey } }); }
  ...
  function togglePrivacy() { privacy = !privacy; client?.sendCmd({ op: 'privacy', enabled: privacy }); }
  ...
  function kp(k: string) {
    if (k === 'Space') sendType(' ');
    else if (['Backspace','Enter','Tab','Caps','Shift','Ctrl','Alt','Win','Menu'].includes(k)) client?.sendCmd({ op: 'key', code: k, down: true });
    else sendType(k);
  }
...
<!-- Control.svelte 与 Connect.svelte 共享逻辑，进入 Control 时如未传 deviceCode，则等价于 Connect 表单页 -->
    ...
    <aside class="hidden md:flex flex-col border-l border-dark-border bg-dark-panel p-3 gap-3 text-sm overflow-auto">
      ...
      <div>
        ...
        <select class="input" bind:value={codec} onchange={sendConfig}>
          <option value="h264">H.264</option><option value="vp9">VP9</option>
          <option value="av1">AV1</option><option value="vp8">VP8</option>
        </select>
      </div>
      ...
      <div class="space-y-1">
        {#each kbRows as row}
          <div class="flex gap-1 justify-center">
            {#each row as k}<button class="btn-ghost btn-sm min-w-[36px]" onclick={() => kp(k)}>{k}</button>{/each}
          </div>
        {/each}
      </div>
    </div>
